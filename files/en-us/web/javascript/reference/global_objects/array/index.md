...
- **JavaScript arrays are zero-indexed**: the first element of an array is at index `0`, the second is at index `1`, and so on — and the last element is at the value of the array's {{jsxref("Array.length", "length")}} property minus `1`.

- **JavaScript array-copy operations create shallow copies** (just as copy operations with _any_ JavaScript objects create shallow copies).
...
In each case, the new array, `fruitsCopy`, is a shallow copy of the `fruits` array — so, changing the `fruits` array also causes the `fruitsShallowCopy` array to change.
